...
/**
 * ToolUserEditor
 * - NEW: pick tools_global type + optional instance label + status
 * - EDIT: edit instance label + status
 *
 * Keep it small: build fix first, no feature creep.
 */
export default function ToolUserEditor({
  toolUser, // tools_user row or null for NEW
  vault = [],
  busy = false,
  onCancel,
  onSave,
    ...
    <Card>
      <div style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
          ...
          <SmallButton disabled={busy || (isNew && !toolGlobalId)} onClick={handleSave}>
            Save
          </SmallButton>
        </div>
      </div>
